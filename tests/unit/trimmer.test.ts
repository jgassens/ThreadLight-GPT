...
describe("trimmer", () => {
  ...
  it("trims a simple active path to the last visible turns", () => {
    ...
    expect(mappingKeys(result.data)).toEqual(["node-5", "node-6", "node-7", "node-8", "node-9", "root"]);
  });
  ...
  it("does not count hidden roles as visible turns", () => {
    const fixture = createLinearConversation(["user", "tool", "assistant", "thinking", "assistant"]);
    const result = trimConversationData(fixture, 20);
  ...
  it("repairs parent and children links for the kept active path", () => {
    ...
    const mapping = result.data.mapping as Record<string, { parent: string | null; children: string[] }>;
